...
svga_mode_info_t *vbe_info_block;
...
void kernel_main(unsigned long ebx) {
  ...
  vbe_info_block = (svga_mode_info_t *) multiboot_grub_info->vbe_mode_info;
  vbe_control_block = (VbeInfoBlock *) multiboot_grub_info->vbe_control_info;
  printf("magic: %s %u\n", vbe_control_block->VbeSignature, vbe_info_block->screen_width);
  main();
}
